...
// Runs first in `npm test` (tests/harness/run.js); standalone: node tests/harness/check-boundaries.js
...
const MANIFEST = {
  ...
  'dev.js': [
    ...
    'devToggleLayoutDebug', 'devToggleUnlocks', 'fetchDevStats', 'fetchDevices', 'fetchRetention',
...
function check() {
  ...
  for (const f of files) if (!(f in MANIFEST)) problems.push(`${f}: new src file — add it to MANIFEST in tests/harness/check-boundaries.js`);
  ...
  try {
    ...
    fs.writeFileSync(path.join(__dirname, '..', '..', '.claude', 'SYMBOLS.md'),
      '# Symbol index (generated by tests/harness/check-boundaries.js on every `npm test` — do not edit)\n\n' +
